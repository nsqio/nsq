...
func (self *NsqdCoordinator) catchupFromLeader(topicInfo TopicPartionMetaInfo) {
	...
	for {
		...
		for i, l := range logs {
			...
			newMsg, err := nsqd.DecodeMessage(d)
			if err != nil {
				coordLog.Infof("Failed to decode message: %v", err)
				return
			}
			err = localTopic.PutMessageOnReplica(newMsg, nsqd.BackendOffset(l.MsgOffset))
			if err != nil {
...
func (self *NsqdCoordinator) PutMessageToCluster(topic *nsqd.Topic, body []byte) error {
	...
	id, offset, totalCnt, putErr := topic.PutMessageNoLock(msg)
	if putErr != nil {
