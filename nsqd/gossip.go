...
func (n *NSQD) serfMemberJoin(ev serf.Event) {
	...
	n.logf(LOG_DEBUG, "MEMBER EVENT: %+v - members: %+v", memberEv, memberEv.Members)
	for _, member := range memberEv.Members {
		...
		n.logf(LOG_INFO, "DB: member(%s) REGISTER %s", producer.ID, r)
	}
}
	...
	for _, member := range memberEv.Members {
		...
		for _, r := range registrations {
			if removed, _ := n.rdb.RemoveProducer(r, member.Name); removed {
				n.logf(LOG_DEBUG, "DB: member(%s) UNREGISTER %s", member.Name, r)
			}
		}
	}
}
...
func (n *NSQD) serfUserEvent(ev serf.Event) {
	...
	if err != nil {
		n.logf(LOG_ERROR, "failed to Unmarshal gossipEvent - %s", err)
		return
	}
	...
	if !found {
		n.logf(LOG_ERROR, "received gossipEvent for unknown node - %s", userEv.Name)
		return
	}
...
func (n *NSQD) gossipHandleCreateEvent(operation byte,
	...
	for _, r := range registrations {
		if n.rdb.AddProducer(r, producer) {
			n.logf(LOG_INFO, "DB: member(%s) REGISTER %s", gev.Name, r)
		}
		if operation == '=' && n.rdb.TouchProducer(r, producer.ID) {
			n.logf(LOG_INFO, "DB: member(%s) TOUCH %s", gev.Name, r)
		}
	}
}
...
func (n *NSQD) gossipHandleDeleteEvent(operation byte,
	...
	if gev.Channel != "" {
		...
		if removed {
			n.logf(LOG_INFO, "DB: member(%s) UNREGISTER %s", gev.Name, r)
		}
	...
	for _, r := range registrations {
		...
		if removed, _ := n.rdb.RemoveProducer(r, producer.ID); removed {
			...
			n.logf(LOG_WARN, "client(%s) unexpected UNREGISTER %s", gev.Name, r)
		}
	}
	...
	if removed, _ := n.rdb.RemoveProducer(r, producer.ID); removed {
		n.logf(LOG_INFO, "DB: client(%s) UNREGISTER %s", gev.Name, r)
	}
}
	...
	for {
		...
		case ev := <-n.serfEventChan:
			...
			case serf.EventMemberUpdate:
				// nothing
			default:
				n.logf(LOG_WARN, "un-handled Serf event: %#v", ev)
			}
...
func (n *NSQD) gossipLoop() {
	...
	if len(n.getOpts().GossipSeedAddresses) > 0 {
		for {
			...
			if err != nil {
				n.logf(LOG_ERROR, "failed to join serf - %s", err)
				select {
			...
			if num > 0 {
				n.logf(LOG_INFO, "SERF: joined %d nodes", num)
				break
			}
		}
	}
	...
	for {
		...
		case <-regossipTicker.C:
			n.logf(LOG_INFO, "SERF: re-gossiping")
			...
			for _, topicStat := range stats {
				if len(topicStat.Channels) == 0 {
					...
					if err != nil {
						n.logf(LOG_ERROR, "failed to send Serf user event - %s", err)
					}
					continue
				}
...
exit:
	regossipTicker.Stop()
	n.logf(LOG_INFO, "GOSSIP: exiting")
}
