...
with an Auth Server that conforms to the auth protocol.

**Note:** It is expected when using authorization that only the nsqd TCP protocol is exposed to external clients, not the http or https endpoints.

The Auth Server must accept a request on `/auth?remote_ip=...&tls=...&auth_secret=...` and return a response in the following format.
...
Note that topic and channel names *must* be regexes. Nsqd will cache the response for up to the TTL duration and will re-request authorization on that interval.
