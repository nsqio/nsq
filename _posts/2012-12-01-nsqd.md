...
To configure nsqd to require authorization you need to specify `-auth-http-address=host:port` 
with an Auth Server that conforms to the Auth HTTP protocol.

**Note:** It is expected when using authorization that only the nsqd TCP protocol is exposed to external clients, not the http or https endpoints. See note below about exposing stats and lookup to clients with auth.

The Auth Server must accept a HTTP request on `/auth?remote_ip=...&tls=...&auth_secret=...` and return a response in the following format.
...
It is expected that in most cases Auth will be used with TLS to secure sensitive auth secrets passsed from clients to nsqd. Communication between nsqd and the Auth Server is expected to be over a trusted network. If you an auth server is making auth choices soley on remote IP information clients can use a placeholder string (like `.`) to trigger the AUTH command which the auth server then ignores.

A helper server to expose nsqlookup and nsqd `/stats` data to a clients filtered by permissions from the Auth Server can be found in [nsqauthfilter](https://github.com/jehiah/nsqauth-contrib#nsqauthfilter)

When using the nsq command line utilities, authorization can be used via `-reader-opt` argument.
