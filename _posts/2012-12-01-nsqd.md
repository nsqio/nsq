...
Monitoring endpoint, should return `OK`. It returns a 500 if it is not healthy.  At the moment, the only "unhealthy" state would be if it failed to write messages to disk when overflow occurred.
